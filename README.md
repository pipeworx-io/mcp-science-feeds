...
`tools/list` at `https://gateway.pipeworx.io/science-feeds/mcp` returns the tools in the table
...
```bash
curl -X POST https://gateway.pipeworx.io/v1/tools/science_feeds_list_feeds \
  -H 'Content-Type: application/json' \
  -d '{"category":"science"}'
```

No account needed for the first calls. Inspect any tool: `GET https://gateway.pipeworx.io/v1/tools/science_feeds_list_feeds`. Find one: `POST https://gateway.pipeworx.io/v1/tools/search_packs` with `{"query":"..."}`.
...
```json
{
  "mcpServers": {
    "science-feeds": {
      "command": "npx",
      "args": ["-y", "@pipeworx/mcp-science-feeds"]
    }
  }
}
```
...
```bash
npx -y @pipeworx/mcp-science-feeds
```
...
```
ask_pipeworx({ question: "your question about Science Feeds data" })
```
